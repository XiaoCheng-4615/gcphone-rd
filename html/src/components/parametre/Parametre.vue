<template>
  <div class="phone_app">
    ...
    <div class='phone_content elements'>
      <div class='element' v-for='(elem, key) in paramList' v-bind:class="{ select: key === currentSelect }"
        v-bind:key="key" @click.stop="onPressItem(key)">
...
export default {
  ...
  computed: {
    ...
    paramList() {
      ...
      return [
        { icons: 'fas fa-phone', title: this.IntlString('APP_CONFIG_MY_MUNBER'), value: this.myPhoneNumber },
        { icons: 'far fa-image', title: this.IntlString('APP_CONFIG_WALLPAPER'), value: this.backgroundLabel, onValid: 'onChangeBackground', values: this.config.background },
        { icons: 'fas fa-mobile-alt', title: this.IntlString('APP_CONFIG_CASE'), value: this.coqueLabel, onValid: 'onChangeCoque', values: this.config.coque },
        { icons: 'far fa-bell', title: this.IntlString('APP_CONFIG_SOUND'), value: this.sonidoLabel, onValid: 'onChangeSonido', values: this.config.sonido },
        { icons: 'fas fa-magnifying-glass', title: this.IntlString('APP_CONFIG_ZOOM'), value: this.zoom, onValid: 'setZoom', onLeft: this.ajustZoom(-1), onRight: this.ajustZoom(1), values: { /* ... */ } },
        { icons: 'fas fa-volume-low', title: this.IntlString('APP_CONFIG_VOLUME'), value: this.valumeDisplay, onValid: 'setPhoneVolume', onLeft: this.ajustVolume(-0.01), onRight: this.ajustVolume(0.01), values: { /* ... */ } },
        { icons: 'fas fa-globe', title: this.IntlString('APP_CONFIG_LANGUAGE'), onValid: 'onChangeLanguages', values: { /* ... */ } },
        { icons: 'fas fa-arrow-pointer', title: this.IntlString('APP_CONFIG_MOUSE_SUPPORT'), onValid: 'onChangeMouseSupport', values: { /* ... */ } },
        { icons: 'fas fa-triangle-exclamation', color: '#ee3838', title: this.IntlString('APP_CONFIG_RESET'), onValid: 'resetPhone', values: { /* ... */ } }
      ]
    },
    valumeDisplay() {
